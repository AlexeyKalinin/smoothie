...
namespace Smoothie
{
    ...
    {
        public static float Step(float current, float target, ref float velocity, float speed, float elasticity)
        {
            float dt = Time.deltaTime;
            float k = speed * 0.1f;
            float damping = 1 + speed * elasticity;

            velocity = ETween.Step(velocity, 0, damping);
            velocity += (target - current) * k;

            return current + velocity * dt;   
        }

        public static Vector2 Step(Vector2 current, Vector2 target, ref Vector2 velocity, float speed, float elasticity)
        {
            float dt = Time.deltaTime;
            float k = speed * 0.1f;
            float damping = 1 + speed * elasticity;

            velocity = ETween.Step(velocity, Vector2.zero, damping);
            velocity += (target - current) * k;

            return current + velocity * dt;
        }

        public static Vector3 Step(Vector3 current, Vector3 target, ref Vector3 velocity, float speed, float elasticity)
        {
            float dt = Time.deltaTime;
            float k = speed * 0.1f;
            float damping = 1 + speed * elasticity;

            velocity = ETween.Step(velocity, Vector3.zero, damping);
            velocity += (target - current) * k;

            return current + velocity * dt;
        }

        public static Quaternion Step(Quaternion current, Quaternion target, ref Vector4 velocity, float speed, float elasticity)
        {
            float dt = Time.deltaTime;
            float k = speed * 0.1f;
            float damping = 1 + speed * elasticity;
            Vector4 v_current = current.ToVector4();
            Vector4 v_target = target.ToVector4();

            velocity = ETween.Step(velocity, Vector4.zero, damping);
            velocity += (v_target - v_current) * k;

            return (v_current + velocity * dt).ToNormalizedQuaternion();
        }
    }
}